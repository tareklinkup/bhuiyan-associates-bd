...
class Sales extends CI_Controller
{
    ...
    public function getSales()
    {
        ...
        $sales = $this->db->query("
            ...
            left join tbl_employee e on e.Employee_SlNo = sm.employee_id
            left join tbl_brunch br on br.brunch_id = sm.SaleMaster_branchid
            left join tbl_driver d on d.id = sm.driver_id
            where sm.SaleMaster_branchid = '$branchId'
            and sm.Status <> 'd'
            $clauses
            order by sm.SaleMaster_SlNo desc
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                // if ($data->sales->Status == 'p') {
                //     $check_inven = $this->db->query("select * from tbl_currentinventory ci where ci.product_id = ? and ci.branch_id = ?", [$cartProduct->productId, $this->session->userdata('BRANCHid')])->row();
    ...
    public function approveSales()
    {
        ...
        try {
            ...
            foreach ($saleDetails as $key => $product) {
                $check_inven = $this->db->query("select * from tbl_currentinventory ci where ci.product_id = ? and ci.branch_id = ?", [$product->Product_IDNo, $this->session->userdata('BRANCHid')])->row();
                if ($check_inven) {
                    //update stock
                    $this->db->query("
                        update tbl_currentinventory 
