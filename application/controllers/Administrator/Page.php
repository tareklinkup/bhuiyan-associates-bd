...
<?php
    ...
    public function diver() {
        ...
        $data['title'] = "Add Diver";
        $data['content'] = $this->load->view('Administrator/diver', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getDiver() {
        $divers = $this->db->query("select * from tbl_driver where status ='a'")->result();
        echo json_encode($divers);
    }
    ...
    public function addDiver() {
        ...
        try {
            ...
            $checkPhone = $this->db->query("select * from tbl_driver where status = 'a' and phone = ?", $data->phone)->num_rows();
            if($checkPhone > 0) {
                $res->success = false;
                $res->message = 'This phone number already diver insert';
                echo json_encode($res);
                exit;
            }
            ...
            $this->db->insert('tbl_driver', $diver);

            $res->success = true;
            $res->message = 'Driver insert successfully';
        } catch (\Exception $ex) {
            $res->success = false;
    ...
    public function updateDiver() {
        $res = new stdClass;

        try {
            $data = json_decode($this->input->raw_input_stream);

            $checkPhone = $this->db->query("select * from tbl_driver where status = 'a' and phone = ? and id !=?", [$data->phone, $data->id])->num_rows();
            if($checkPhone > 0) {
                $res->success = false;
                $res->message = 'This phone number already driver insert';
                echo json_encode($res);
                exit;
            }
            ...
            $this->db->where('id', $data->id)->update('tbl_driver', $diver);
    ...
    public function deleteDiver() {
        ...
        try {
            ...
            $this->db->set('status', 'd')->where('id', $data->driverId)->update('tbl_driver');
            
            $res->success = true;
            $res->message = 'Driver deleted successfully';
        } catch (\Exception $ex) {
            $res->success = false;
            $res->message = $ex->getMessage();
        }
