...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            ...
            $badDebtClause = " and CPayment_date between '$data->dateFrom' and '$data->dateTo'";
        }

        $customerClause = '';
        if ($data->customer != null && $data->customer != '') {
            $customerClause = " and CPayment_customerID = '$data->customer'";
        }
        ...
        $result = $this->db->query("
            ...
            (
                select ifnull(sum(pm.PurchaseMaster_loadingCost), 0) 
                from tbl_purchasemaster pm
                where pm.PurchaseMaster_BranchID = '" . $this->session->userdata('BRANCHid') . "'
                and pm.status = 'a'
                $purchaseClause
            ) as purchase_loading_cost,
            ...
            (
                SELECT ifnull(SUM(CPayment_discount),0) 
                FROM tbl_customer_payment
                WHERE CPayment_status = 'a'
                $customerClause
                and CPayment_brunchid = '" . $this->session->userdata('BRANCHid') . "'
                $badDebtClause
            ) as bad_debt_amount
        ")->row();
